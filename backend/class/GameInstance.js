...
export class OnlineMatchmaking {
  ...
  registerPlayer(email, connection, gameNoOfPlayers, request, username, tournamentContext = {}, gameType) {   
    ...
    const hasTournamentContext = tournamentContext && tournamentContext.tournamentId;

    const playersWaitingForSameMode = nonPlayingPlayers.filter((player) => {
      const playerHasTournamentContext = player.tournamentContext && player.tournamentContext.tournamentId;
