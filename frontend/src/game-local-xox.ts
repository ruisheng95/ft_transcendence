...
export function xox_game_setup()
{
	...
	start_button.addEventListener("click", () => {
		...
		//start game logic all moved here
		const socket = WS.getInstance(`${import.meta.env.VITE_SOCKET_URL}/ws_xox`);

		if (socket.readyState === WebSocket.OPEN)
			sendStartGame();
		else
        	socket.addEventListener("open", sendStartGame, { once: true });

		//{ once: true } is an option for addEventListener that makes the event listener automatically remove itself after it fires once.
		...
		{
			if(!p1_name || !p2_name)
				throw new Error("xox start game elements not found");
